...
def yelp_bphoto_cdn_url(photo_id: str, size: str = "348s") -> str:
    """
    Public Yelp CDN URL for a ``photo_id`` from Yelp Academic Dataset ``photos.json``.
    Does not require local JPGs; browser loads the thumbnail directly.
    """
    pid = str(photo_id).strip()
    if not pid:
        raise ValueError("photo_id required")
    return f"https://s3-media0.fl.yelpcdn.com/bphoto/{pid}/{size}.jpg"


class YelpPhotoTarReader:
    """Keep one tar handle for repeated extractfile (long-lived process)."""
