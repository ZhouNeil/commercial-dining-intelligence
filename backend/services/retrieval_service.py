"""
Thin search service: TouristRetrieval + parse_query (matches the `/search` request body).
"""
...
# Placeholder images when Yelp photo_id is missing (same set as frontend @error fallback)
...
class RetrievalSearchService:
    def __init__(self, repo_root: Optional[Path] = None):
        ...
        self._retrieval = TouristRetrieval(
            ...
            # Do not restrict to the top-11 metro whitelist; pool size still capped by max_businesses / per-state caps
            restrict_index_cities=False,
    ...
    def search(
        ...
        *,
        ...
        """
        Aligns with the client: Discover (broad) or Refine (NL + cuisines + weights + pool),
        with optional v2 reranking in the pool from likes/dislikes.
        """
